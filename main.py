...
if datetime.now().strftime("%A") == "Monday":
    YESTERDAY = (datetime.now() - timedelta(3)).strftime("%A, %B %d, %Y")
else:
    YESTERDAY = (datetime.now() - timedelta(1)).strftime("%A, %B %d, %Y")
...
def select_date(driver):
    driver.find_element(By.CSS_SELECTOR, f"[title^='{YESTERDAY}']").click()
...
def write_to_csv(array):
    ...
    # location = '\\SERVER\\jdjl\\interest-nz\\interest.co.nz\\chart_data\\interestrates\\bkbm-1mth.csv'
    for x in range(1, 7):
        with open(f'C:\\Users\\ken\\Desktop\\Book{x}.csv', 'a', newline='') as target_csv:
